...
print("\n")
ndimo, ndimd, ind_offdiag, ind_diag = matogen(30, 21, 15, 10, 2) #(8, 7, 4, 3, 2)
...
from myModule import w3jmatlabC, dlkmC
#from dlkmo import *
...
def SpinHamiltonianSuperoperatorDiag(ind_arr, B0, psi, I, g, A, ald, bed, gad, alm, bem, gam):
    ...
    def R_g(l,jK1,jK2,L1,L2,K1,K2):
        return (w3jmatlabC(L1,l,L2,K1,K2-K1,-K2)) * G_g(l,jK1,jK2,K1-K2) +\
               jK2 * par(L2+K2) * (w3jmatlabC(L1,l,L2,K1,-K2-K1,K2)) * G_g(l,jK1,jK2,K1+K2)
    ...
    def Ax_a(l,pI1,pI2,qI1,qI2):
        ...
        S_A = (dpI == 0) * pI1/2 + (dpI != 0) * -dqI * K_I/math.sqrt(8)
        return (abs(dpI) == abs(dqI)) * par(dpI) * np.sqrt(2*l+1) * (w3jmatlabC(1,1,l,0,dpI,-dpI)) * S_A
            ...
            if abs(dpI) <=1 and abs(dpI)==abs(dqI) and abs(M2-M1) <=2:
                mat[i,j] = NormFacL(L1,L2) * NormFacK(K1,K2) * par(M1+K1) * \
                        sum([R_a(l,jK1,jK2,L1,L2,K1,K2) * (w3jmatlabC(L1,l,L2,M1,M2-M1,-M2)) * \
                             Ax_a(l,pI1,pI2,qI1,qI2) * dlkmC(l,dpI,M1-M2,0,psi,0) for l in [0,2]]) 
                mat[j,i] = mat[i,j]
...
mm = SpinHamiltonianSuperoperatorDiag(ind_diag, B0, psi, In, [2.0087,2.0057,2.0021], [6,6,36], ald,bed,gad, alm,bem,gam)
stp = time.time()
print(stp-stt)
