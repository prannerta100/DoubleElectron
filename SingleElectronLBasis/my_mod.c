...
if(abs(dpI) != abs(dqI) || abs(dpI) != abs(dqI) || dpS*dpI != dqS*dqI) //first few deltas in the expression
    return 0.0+0.0*I;
tmp= qI1 * dqI + pI1 * dpI;
...
if(dpS==0){
    ...
    if(dpI==0){
        S_A = -(pI1*dpS+qI1*dqS) * K_I/sqrt(8);
    }
    else{
    	S_A = dpS * dqI * K_I /2;
    }
...
static PyMethodDef myMethods[] = {
    ...
    { "Ax_aC", Ax_aDiagDiff, METH_VARARGS, "Ax_aDiff 6 args l,pI1,pI2,qI1,qI2,I" },
    { "Ax_gC", Ax_aOffDiagDiff, METH_VARARGS, "Ax_aDiff 6 args l,pI1,pI2,qI1,qI2,I" },
    { NULL, NULL, 0, NULL }
};
