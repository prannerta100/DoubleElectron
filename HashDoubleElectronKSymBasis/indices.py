#hash map (dict) to map index back into tuples
...
def indgen(Lemax, Lomax, Kmax, Mmax, ipnmx, g, A, I2, angd, angm, angdip, psi):
    ...
    #!!!NEW!!! I added gadip, as gadip is like gad
...
#we will get into the loop, and then just add a subloop for the 2 other electrons
    ind_offdiag_rev = {}
    ind_diag_rev = {}
    ind_Plus1_rev = {} #dictionary
    ind_Plus2_rev = {} #dictionary
    ind_Minus1_rev = {} #dictionary
...
#                for jM in range(jMmin,1+1,2):
                for M in range(-min(Mmax, L),min(Mmax, L)+1):
                    ...
                    for pIa in range(pImin, pImax+1):
                        ...
                        for qIa in range(qImina, qImaxa+1, 2):
                            ...
                            ind_offdiag_rev[(L,M,K,jK,pIa,qIa,1,0)] = ndimo #jM removed

                            ndimo += 1

                            #Diag
                            ind_diag.append([L,M,K,jK,pIa,qIa,0,1]) #jM removed
                            ind_diag_rev[(L,M,K,jK,pIa,qIa,0,1)] = ndimd #jM removed

                            ind_diag.append([L,M,K,jK,pIa,qIa,0,-1]) #jM removed
                            ind_diag_rev[(L,M,K,jK,pIa,qIa,0,-1)] = ndimd+1 #jM removed
                            ...
                            for pIb in range(pImin, pImax+1):
                                ...
                                for qIb in range(qIminb, qImaxb+1, 2):
                                    ...
                                    ind_Plus1_rev[(L,M,K,jK,pIa,qIa,1,0,pIb,qIb,0,1)] = ndimPlus1 #jM removed

                                    ind_Plus1.append([L,M,K,jK,pIa,qIa,1,0,pIb,qIb,0,-1]) #jM removed
                                    ind_Plus1_rev[(L,M,K,jK,pIa,qIa,1,0,pIb,qIb,0,-1)] = ndimPlus1+1 #jM removed

                                    ind_Plus1.append([L,M,K,jK,pIa,qIa,0,1,pIb,qIb,1,0]) #jM removed
                                    ind_Plus1_rev[(L,M,K,jK,pIa,qIa,0,1,pIb,qIb,1,0)] = ndimPlus1+2 #jM removed

                                    ind_Plus1.append([L,M,K,jK,pIa,qIa,0,-1,pIb,qIb,1,0]) #jM removed
                                    ind_Plus1_rev[(L,M,K,jK,pIa,qIa,0,-1,pIb,qIb,1,0)] = ndimPlus1+3 #jM removed
                                    ...
                                    ind_Plus2_rev[(L,M,K,jK,pIa,qIa,1,0,pIb,qIb,1,0)] = ndimPlus2 #jM removed
                                    ...
                                    ind_Minus1_rev[(L,M,K,jK,pIa,qIa,-1,0,pIb,qIb,0,1)] = ndimMinus1 #jM removed

                                    ind_Minus1.append([L,M,K,jK,pIa,qIa,-1,0,pIb,qIb,0,-1]) #jM removed
                                    ind_Minus1_rev[(L,M,K,jK,pIa,qIa,-1,0,pIb,qIb,0,-1)] = ndimMinus1+1 #jM removed

                                    ind_Minus1.append([L,M,K,jK,pIa,qIa,0,1,pIb,qIb,-1,0]) #jM removed
                                    ind_Minus1_rev[(L,M,K,jK,pIa,qIa,0,1,pIb,qIb,-1,0)] = ndimMinus1+2 #jM removed

                                    ind_Minus1.append([L,M,K,jK,pIa,qIa,0,-1,pIb,qIb,-1,0]) #jM removed
                                    ind_Minus1_rev[(L,M,K,jK,pIa,qIa,0,-1,pIb,qIb,-1,0)] = ndimMinus1+3 #jM removed
    ...
    Lstarts_Plus2.append(ndimPlus2)
    Lstarts_Minus1.append(ndimMinus1)
    print('ind_diag_rev[(1, -1, 1, 1, -2, 0, 0, 1)]',ind_diag_rev[(1, -1, 1, 1, -2, 0, 0, 1)])
    return ndimo, ndimd, ndimPlus1, ndimPlus2, ndimMinus1, \
           ...
           Llist, ind_diag_rev #(just a list of L's, doesn't matter whether it is single-electron or double-electron coherence) 
