...
def SpinHamiltonianSuperoperatorDiag(ind_arr, B0, psi, I, g, A, ald, bed, gad, alm, bem, gam, Llist, Lstarts_diag, ind_diag_rev):
...
#        return 0 #(no B0 term in diag space)
    ...
    for i in range(ndim): #ndim is ndimd
        ...
        #left limit
        for L2 in range(L1-2,L1+3):  #range(ndim) #define band, this will really speed up things! Think about the nbnd in CW nlsl matrll.f
            if L2 in Llist:
                for M2 in range(M1-2,M1+3):
                    if(abs(M2)<=L2):
                        for jK2 in [1]: #nein gut, I must change this to the original
                            for K2 in range(max(0,K1-2),K1+3):
                                if abs(K2)<=L2 and (K2!=0 or par(L2)==jK2):
                                    for pI2 in range(-2*I,2*I+1):
                                        qImax=2*I - abs(pI2)
                                        qImin=-qImax
                                        for qI2 in range(qImin, qImax+1, 2):
                                            for pS2,qS2 in [(0,1),(0,-1)]:
                                        #if i==7 or j==7:
                                        #    print(left_j,right_j)
                                                j = ind_diag_rev[(L2,M2,K2,jK2,pI2,qI2,pS2,qS2)]
